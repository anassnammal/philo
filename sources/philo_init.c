...
static bool philo_init_mem()
{
	...
	if (philo_data->tid == NULL)
		return (false);
	philo_data->locks = (t_lock *)malloc(sizeof(t_lock) * (n + 3));
...
static bool	philo_init_mtx()
{
	...
	n = ((t_var *)philo_get(PHILO_DATA_PRM))->n_philo + 3;
	i = 0;
	while (i < n)
	{
...
static void	philo_init_ptrs()
{
	...
	while (i < n)
	{
		...
		philo_data->philos[i].count_meal = 0;
...
void *philo_get(uint8_t op)
{
	...
	if (op & PHILO_LOCK_PRT)
		return (&philo_data.locks[philo_data.params->n_philo]);
	if (op & PHILO_LOCK_LML)
		return (&philo_data.locks[philo_data.params->n_philo + 1]);
	if (op & PHILO_LOCK_MCT)
		return (&philo_data.locks[philo_data.params->n_philo + 2]);
    return (&philo_data);
}